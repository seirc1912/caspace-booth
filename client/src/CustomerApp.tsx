...
import { downloadBlob } from './features/orders/services/downloadBlob'
...
export function CustomerApp() {
  ...
  const saveAndContinue = async () => {
    if (!booth.slots.length || booth.slots.some((slot) => !slot) || !booth.room || downloading) return
    setDownloading(true)
    try {
  ...
  if (pathname === '/rooms') return isValidPhoneNumber(booth.phoneNumber) ? <RoomSelectionPage onBack={() => navigate('/')} onSelect={(roomId) => { booth.selectRoom(roomId); setOrderDraft(null); setOrderItems({}); setFramePreviews({}); navigate('/editor') }} rooms={booth.rooms} templateCount={(roomId) => booth.templates.filter((template) => template.roomId === roomId).length} /> : <HomePage onContinue={(phoneNumber) => { booth.setPhoneNumber(phoneNumber); navigate('/rooms') }} phoneNumber={booth.phoneNumber} />
