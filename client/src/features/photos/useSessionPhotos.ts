...
export function useSessionPhotos(session: CustomerSession | null, onPhotos: (photos: PhotoAsset[]) => void, onError: (message: string) => void) {
  useEffect(() => {
    if (!session) return
    let active = true
    fetch(`/api/photos/session/${session.sessionId}`, { headers: { 'X-Session-Token': session.token } })
    ...
    const socket = io({ auth: { sessionId: session.sessionId, token: session.token }, transports: ['websocket', 'polling'] })
