...
type MeasurementUnit = 'px' | 'cm' | 'inch'
type InspectorGeometry = Pick<TemplateSlot, 'x' | 'y' | 'width' | 'height' | 'rotation'> & { borderRadius?: number }
const pixelsPerUnit: Record<MeasurementUnit, number> = { px: 1, cm: 37.79527559, inch: 96 }
const displayMeasurement = (pixels: number, unit: MeasurementUnit) => Number((pixels / pixelsPerUnit[unit]).toFixed(unit === 'px' ? 2 : unit === 'cm' ? 2 : 3))

function GeometryControls({ name, geometry, onChange }: { name: string; geometry: InspectorGeometry; onChange: (changes: Partial<InspectorGeometry> & { name?: string }) => void }) {
  const [unit, setUnit] = useState<MeasurementUnit>('px')
  return <div>
    <div className="border-b border-stone-100 pb-3"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-violet-600">Selected layer</p><label className="field mt-2 block">Layer name<input className={inputClass} onChange={(event) => onChange({ name: event.target.value })} value={name} /></label></div>
    <div className="grid gap-4 py-4">
      <fieldset><legend className="mb-2 text-xs font-bold text-stone-950">Position</legend><div className="grid grid-cols-2 gap-2"><NumberField label="X" value={geometry.x} onChange={(x) => onChange({ x })} /><NumberField label="Y" value={geometry.y} onChange={(y) => onChange({ y })} /></div></fieldset>
      <fieldset><div className="mb-2 flex items-center justify-between gap-2"><legend className="text-xs font-bold text-stone-950">Size</legend><label className="flex items-center gap-2 text-[11px] font-semibold text-stone-500">Unit<select aria-label="Measurement unit" className="min-h-8 rounded-lg border border-stone-200 bg-white px-2 text-xs text-stone-700" onChange={(event) => setUnit(event.target.value as MeasurementUnit)} value={unit}><option value="px">px</option><option value="cm">cm</option><option value="inch">inch</option></select></label></div><div className="grid grid-cols-2 gap-2"><NumberField label="Width" value={displayMeasurement(geometry.width, unit)} onChange={(width) => onChange({ width: width * pixelsPerUnit[unit] })} /><NumberField label="Height" value={displayMeasurement(geometry.height, unit)} onChange={(height) => onChange({ height: height * pixelsPerUnit[unit] })} /></div></fieldset>
    </div>
    <AccordionSection defaultOpen title="Transform"><div className="grid grid-cols-2 gap-2"><NumberField label="Rotation" value={geometry.rotation} onChange={(rotation) => onChange({ rotation })} />{geometry.borderRadius !== undefined ? <NumberField label="Border radius" value={geometry.borderRadius} onChange={(borderRadius) => onChange({ borderRadius: Math.max(0, borderRadius) })} /> : null}</div></AccordionSection>
  </div>
}
...
function ElementInspector({ element, onChange, onAsset }: { element: TemplateElement; onChange: (changes: Partial<TemplateElement>) => void; onAsset: (event: ChangeEvent<HTMLInputElement>) => void }) {
  ...
  const geometry = <GeometryControls geometry={element} name={element.name} onChange={onChange} />
