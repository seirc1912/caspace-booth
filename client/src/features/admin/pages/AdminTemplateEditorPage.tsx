  ...
  return <div className="min-w-0">
    ...
    <div className="grid gap-4 xl:grid-cols-[18rem_minmax(0,1fr)_19rem]">
      ...
      <aside className="grid content-start gap-4">
        ...
        {selectedSlot ? <FrameDetails onChange={(changes) => changeSlot(selectedSlot.id, changes)} slot={selectedSlot} /> : null}
...
function FrameDetails({ slot, onChange }: { slot: TemplateSlot; onChange: (changes: Partial<TemplateSlot>) => void }) {
  ...
  return <section className="panel"><h2 className="text-sm font-bold">Frame styling</h2><div className="mt-3 grid gap-3"><label className="field">Aspect ratio<select className={inputClass} onChange={(event) => setRatio(event.target.value as NonNullable<TemplateSlot['aspectRatio']>)} value={slot.aspectRatio ?? 'free'}><option value="free">Free</option><option value="1:1">1:1</option><option value="3:4">3:4</option><option value="4:3">4:3</option><option value="9:16">9:16</option></select></label><div className="grid grid-cols-2 gap-2"><NumberField label="Photo index" value={slot.photoIndex ?? 0} onChange={(photoIndex) => onChange({ photoIndex: Math.max(0, Math.round(photoIndex)) })} /><NumberField label="Border width" value={slot.borderWidth ?? 0} onChange={(borderWidth) => onChange({ borderWidth: Math.max(0, borderWidth) })} /><NumberField label="Shadow blur" value={shadow.blur} onChange={(blur) => onChange({ shadow: { ...shadow, blur: Math.max(0, blur) } })} /><NumberField label="Shadow X" value={shadow.offsetX} onChange={(offsetX) => onChange({ shadow: { ...shadow, offsetX } })} /><NumberField label="Shadow Y" value={shadow.offsetY} onChange={(offsetY) => onChange({ shadow: { ...shadow, offsetY } })} /></div><div className="grid grid-cols-2 gap-2"><label className="field">Border color<input className="mt-1 h-10 w-full" onChange={(event) => onChange({ borderColor: event.target.value })} type="color" value={slot.borderColor ?? '#000000'} /></label><label className="field">Shadow color<input className="mt-1 h-10 w-full" onChange={(event) => onChange({ shadow: { ...shadow, color: event.target.value } })} type="color" value={shadow.color} /></label></div><div className="grid grid-cols-2 gap-2"><button className="min-h-10 rounded-lg bg-stone-100 text-xs font-bold" onClick={() => onChange({ zIndex: slot.zIndex + 1 })} type="button">Bring Forward</button><button className="min-h-10 rounded-lg bg-stone-100 text-xs font-bold" onClick={() => onChange({ zIndex: Math.max(0, slot.zIndex - 1) })} type="button">Send Backward</button></div></div></section>
}
