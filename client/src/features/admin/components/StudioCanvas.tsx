...
type Geometry = Pick<TemplateSlot, 'x' | 'y' | 'width' | 'height' | 'rotation'>
...
const handleClass = 'absolute z-20 size-3 rounded-[3px] border-2 border-white bg-violet-600 shadow-sm'
...
const ObjectControl = memo(function ObjectControl({ id, name, locked, selected, zoom, geometry, canvas, onCommit, onContextMenu, onDoubleClick, onPreview, onSelect, onGuides }: ObjectControlProps) {
  ...
  return <div className="pointer-events-none absolute z-[200]" data-studio-control={id} style={style}><button aria-label={`Select ${name}`} className={`pointer-events-auto absolute inset-0 border-2 bg-transparent transition-[border-color,box-shadow] duration-100 ${selected ? 'border-violet-600 shadow-[0_0_0_1px_rgba(255,255,255,.9)]' : 'border-transparent hover:border-violet-300'}`} onContextMenu={(event) => { event.preventDefault(); onContextMenu(event, id) }} onDoubleClick={(event) => { event.stopPropagation(); onDoubleClick(id) }} onPointerCancel={finish} onPointerDown={(event) => start(event, 'move')} onPointerMove={move} onPointerUp={finish} style={{ cursor: locked ? 'default' : 'move', touchAction: 'none' }} type="button"><span className="sr-only">{name}</span></button>{selected && !locked ? <>{handles.map((handle) => <button aria-label={`${handle.id} resize ${name}`} className={`${handleClass} ${handle.className} pointer-events-auto`} key={handle.id} onPointerCancel={finish} onPointerDown={(event) => start(event, handle.id)} onPointerMove={move} onPointerUp={finish} style={{ cursor: handle.cursor, touchAction: 'none' }} type="button" />)}<div className="pointer-events-none absolute bottom-full left-1/2 h-7 w-px -translate-x-1/2 bg-violet-600" /><button aria-label={`Rotate ${name}`} className="pointer-events-auto absolute -top-10 left-1/2 size-4 -translate-x-1/2 rounded-full border-2 border-white bg-violet-600 shadow" onPointerCancel={finish} onPointerDown={(event) => start(event, 'rotate')} onPointerMove={move} onPointerUp={finish} style={{ cursor: 'grab', touchAction: 'none' }} type="button" />{angle !== null ? <span className="pointer-events-none absolute -top-16 left-1/2 -translate-x-1/2 rounded bg-stone-950 px-2 py-1 text-[11px] font-bold text-white">{Math.round(angle)}°</span> : null}</> : null}</div>
})
