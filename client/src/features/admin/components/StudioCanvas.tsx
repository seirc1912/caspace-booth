...
const ObjectControl = memo(function ObjectControl({ id, name, locked, selected, cropMode, zoom, geometry, canvas, onCommit, onContextMenu, onDoubleClick, onPreview, onSelect, onGuides }: ObjectControlProps) {
...
interface CropTransform { x: number; y: number; zoom: number; rotation: number }

function CropSlotPreview({ slot, transform, onChange }: { slot: TemplateSlot; transform: CropTransform; onChange: (next: CropTransform) => void }) {
  const drag = useRef<{ x: number; y: number; originX: number; originY: number } | null>(null)
  const photo = samplePhotos[(slot.photoIndex ?? 0) % samplePhotos.length]!
  const down = (event: PointerEvent<HTMLDivElement>) => { event.preventDefault(); event.currentTarget.setPointerCapture(event.pointerId); drag.current = { x: event.clientX, y: event.clientY, originX: transform.x, originY: transform.y } }
  const move = (event: PointerEvent<HTMLDivElement>) => { if (drag.current) onChange({ ...transform, x: drag.current.originX + event.clientX - drag.current.x, y: drag.current.originY + event.clientY - drag.current.y }) }
  const wheel = (event: WheelEvent<HTMLDivElement>) => { event.preventDefault(); onChange({ ...transform, zoom: Math.min(4, Math.max(.25, transform.zoom * Math.exp(-event.deltaY * .002))) }) }
  return <div aria-label={`Crop ${slot.name ?? 'photo slot'}`} className="group relative size-full cursor-grab touch-none overflow-hidden ring-[3px] ring-inset ring-[#0D99FF] active:cursor-grabbing" onPointerCancel={() => { drag.current = null }} onPointerDown={down} onPointerMove={move} onPointerUp={() => { drag.current = null }} onWheel={wheel} style={{ borderRadius: `${slot.borderRadius / Math.min(slot.width, slot.height) * 100}%` }}><img alt="Crop preview" className="pointer-events-none absolute inset-0 size-full object-cover transition-transform duration-75" src={photo.previewSrc ?? photo.src} style={{ transform: `translate(${transform.x}px, ${transform.y}px) rotate(${transform.rotation}deg) scale(${transform.zoom})` }} /><div className="absolute inset-x-1 bottom-1 flex items-center justify-center gap-1 rounded-lg bg-stone-950/80 p-1 text-[9px] font-bold text-white opacity-90"><button aria-label="Zoom out crop" className="rounded px-1.5 py-1 hover:bg-white/15" onClick={(event) => { event.stopPropagation(); onChange({ ...transform, zoom: Math.max(.25, transform.zoom - .1) }) }} type="button">−</button><button aria-label="Rotate crop left" className="rounded px-1.5 py-1 hover:bg-white/15" onClick={(event) => { event.stopPropagation(); onChange({ ...transform, rotation: transform.rotation - 15 }) }} type="button">↶</button><span>Enter to finish</span><button aria-label="Rotate crop right" className="rounded px-1.5 py-1 hover:bg-white/15" onClick={(event) => { event.stopPropagation(); onChange({ ...transform, rotation: transform.rotation + 15 }) }} type="button">↷</button><button aria-label="Zoom in crop" className="rounded px-1.5 py-1 hover:bg-white/15" onClick={(event) => { event.stopPropagation(); onChange({ ...transform, zoom: Math.min(4, transform.zoom + .1) }) }} type="button">+</button></div></div>
}

interface StudioCanvasProps {
  template: PrintTemplate
  cropSlotId: string | null
...
export function StudioCanvas({ template, cropSlotId, selectedSlotIds, zoom, pan, guides, onElementChange, onSlotChange, onSelect, onClearSelection, onDoubleClick, onContextAction, onViewportChange }: StudioCanvasProps) {
  ...
  const preview = useCallback((id: string, draft: Geometry) => {
    ...
    others.forEach((item) => { xTargets.push(item.x, item.x + item.width / 2, item.x + item.width); yTargets.push(item.y, item.y + item.height / 2, item.y + item.height) })
    for (let leftIndex = 0; leftIndex < others.length; leftIndex += 1) {
      for (let rightIndex = leftIndex + 1; rightIndex < others.length; rightIndex += 1) {
        const first = others[leftIndex]!; const second = others[rightIndex]!
        const horizontal = first.x <= second.x ? [first, second] : [second, first]
        const horizontalGap = horizontal[1]!.x - (horizontal[0]!.x + horizontal[0]!.width)
        if (horizontalGap >= 0) others.forEach((item) => xTargets.push(item.x + item.width + horizontalGap, item.x - snappedDraft.width - horizontalGap))
        const vertical = first.y <= second.y ? [first, second] : [second, first]
        const verticalGap = vertical[1]!.y - (vertical[0]!.y + vertical[0]!.height)
        if (verticalGap >= 0) others.forEach((item) => yTargets.push(item.y + item.height + verticalGap, item.y - snappedDraft.height - verticalGap))
      }
    }
