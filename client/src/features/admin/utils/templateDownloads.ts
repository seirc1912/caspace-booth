...
export function downloadTemplateJson(record: AdminTemplateRecord) {
  const safeName = record.template.name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-') || 'template'
  download(new Blob([serializeTemplate(record.template)], { type: 'application/json' }), `${safeName}.json`)
}
