...
import { renderComposition, type ExportProgress, type ImageExportFormat, type RenderAssetCache } from './renderComposition'
...
export async function prepareCompositionDownload({ assetCache, branding, format = 'png', onProgress, slots, template }: DownloadCompositionInput): Promise<PreparedCompositionDownload> {
  if (!slots.length || slots.some((slot) => !slot)) throw new Error('Fill every photo slot before exporting.')
  const rendered = await renderComposition(template, slots, { assetCache, branding, createPreview: false, format, quality: 0.95, onProgress })
