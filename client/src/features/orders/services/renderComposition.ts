...
const loadImage = (source: string) => new Promise<HTMLImageElement>((resolve, reject) => {
  ...
  image.onload = () => image.src && image.complete && Number.isFinite(image.naturalWidth) && Number.isFinite(image.naturalHeight) && image.naturalWidth > 0 && image.naturalHeight > 0
    ? resolve(image)
    : reject(new Error('An export image has invalid dimensions.'))
