...
export function ComposerPage({ template, slots, currentSlot, onBack, onClear, onSave, onFillEmpty, onNext, onShuffle, onCurrentSlotChange, onRemove, onReplace, onTransform, onFitChange, uploadedPhotos, onAddPhotos, onDeletePhoto, onReplacePhoto, photoError, onClearPhotoError, onPhotoError, frameIndex = 0, frameCount = 1, completedFrameIds = [], frameIds = [], onSelectFrame, onDownload, downloading, onPrevious }: ComposerPageProps) {
  ...
  return <PageShell className="pb-36 md:pb-24">
    ...
    {photoError ? <div className="fixed inset-x-4 top-4 z-[70] mx-auto max-h-[calc(100dvh-2rem)] max-w-2xl overflow-auto rounded-2xl bg-rose-600 px-4 py-3 text-sm font-semibold text-white shadow-2xl" role="alert"><pre className="whitespace-pre-wrap break-all font-mono text-xs">{photoError}</pre><div className="mt-3 flex justify-end gap-2"><button className="min-h-10 rounded-xl bg-white/15 px-3 font-bold" onClick={() => void navigator.clipboard.writeText(photoError)} type="button">Copy diagnostics</button><button className="min-h-10 rounded-xl bg-white/15 px-3 font-bold" onClick={onClearPhotoError} type="button">Dismiss</button></div></div> : null}
