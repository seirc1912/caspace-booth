  ...
  return (
    <PageShell className="pb-24">
      <header className="mb-5 flex items-center justify-between">
        ...
        <div className="text-center"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--brand-primary)]">Step 2 of 3</p><h1 className="font-bold">Add your photos</h1></div>
      ...
      <EditorToolbar canContinue={slots.every(Boolean)} onAutoFill={() => onFillEmpty(uploadedPhotos)} onClear={() => { onClear(); onCurrentSlotChange(null) }} onNext={onNext} onRandomFill={onRandomFill} onShuffle={onShuffle} />
      <SourceSheet open={sourceOpen && !galleryOpen} onCancel={() => setSourceOpen(false)} onPhonePhotos={(photos) => { onAddPhotoAssets(photos); placePhotos(photos) }} onSelfBoothPhotos={openGallery} />
      {galleryOpen ? (
        <PhotoGallery
