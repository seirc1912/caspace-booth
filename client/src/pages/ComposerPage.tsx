...
export function ComposerPage({ template, slots, currentSlot, onBack, onClear, onSave, onFillEmpty, onNext, onShuffle, onCurrentSlotChange, onRemove, onReplace, onTransform, onFitChange, uploadedPhotos, onAddPhotos, onDeletePhoto, onReplacePhoto, photoError, onClearPhotoError, onPhotoError, frameIndex = 0, frameCount = 1, completedFrameIds = [], canOrder, frameIds = [], onSelectFrame, onDownload, downloading, onPrevious }: ComposerPageProps) {
  ...
  const dropPhoto = (index: number, photoId: string) => {
    ...
    if (photo) { assign(photo, index); setReplaceSlot(null) }
  }
  ...
  return <PageShell className="pb-[calc(var(--mobile-editor-controls-height,18rem)+1rem)] md:pb-24">
    ...
    <div className="fixed inset-x-0 bottom-0 z-20 bg-white md:contents" ref={bottomControls}>{!crop ? <div className="mx-auto w-full max-w-4xl md:mt-5"><PhotoLibraryDock activePhotoId={activePhotoId} onImageError={onPhotoError} onAdd={onAddPhotos} onSelect={(photo) => { setActivePhotoId(photo.id); if (replaceSlot !== null) { assign(photo, replaceSlot); setReplaceSlot(null) } }} onDelete={deletePhoto} onReplace={onReplacePhoto} photos={uploadedPhotos} usage={usage} /></div> : null}<EditorToolbar canContinue={slots.every(Boolean)} canOrder={canOrder} downloading={downloading} nextLabel="Order" onAutoFill={() => onFillEmpty(uploadedPhotos)} onClear={() => { onClear(); onCurrentSlotChange(null); setCrop(null); setReplaceSlot(null) }} onDownload={onDownload} onNext={onNext} onPrevious={onPrevious} onSave={() => { onSave(); onCurrentSlotChange(null); setCrop(null); setReplaceSlot(null) }} onShuffle={onShuffle} previousDisabled={frameIndex === 0} saved={completedFrameIds.includes(template.id)} /></div>
    {crop ? <CropToolbar fit={slots[crop.index]?.fit ?? 'contain'} onCancel={cancelCrop} onDone={finishCrop} onFitChange={(fit) => onFitChange(crop.index, fit)} onPositionXChange={(x) => onTransform(crop.index, { x })} onPositionYChange={(y) => onTransform(crop.index, { y })} onReset={resetCrop} onZoomChange={(zoom) => onTransform(crop.index, { zoom })} positionX={slots[crop.index]?.transform.x ?? 0} positionY={slots[crop.index]?.transform.y ?? 0} zoom={slots[crop.index]?.transform.zoom ?? 1} /> : null}
