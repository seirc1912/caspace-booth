...
export function ComposerPage({ template, slots, currentSlot, onBack, onClear, onFillEmpty, onNext, onShuffle, onCurrentSlotChange, onRemove, onReplace, onTransform, onFitChange, uploadedPhotos, onAddPhotos, onDeletePhoto, onReplacePhoto, photoError, onClearPhotoError, onPhotoError, frameIndex = 0, frameCount = 1, completedFrameIds = [], frameIds = [], onSelectFrame, onDownload, downloading, onPrevious }: ComposerPageProps) {
  ...
  return <PageShell className="pb-[15.5rem] md:pb-24">
    ...
    <EditorToolbar canContinue={slots.every(Boolean)} downloading={downloading} nextLabel={frameIndex === frameCount - 1 ? 'Finish' : 'Next'} onAutoFill={() => onFillEmpty(uploadedPhotos)} onClear={() => { onClear(); onCurrentSlotChange(null); setCrop(null) }} onDownload={onDownload} onNext={onNext} onPrevious={onPrevious} onShuffle={onShuffle} previousDisabled={frameIndex === 0} />
  </PageShell>
}
