...
export function RoomSummaryPage({ roomName, roomId, phoneNumber, templates, frameSlots, completedFrameIds, previewUrls, onEdit, onSuccess }: RoomSummaryPageProps) {
  ...
  const completedFrames = frames.map((frame, index) => ({ ...frame, index })).filter(({ template, slots }) => completedFrameIds.includes(template.id) && slots.every(Boolean))
  const performDownload = async (index: number) => {
    const frame = frames[index]; if (!frame) return
    await downloadComposition({ branding, slots: frame.slots, template: frame.template })
  }
  const downloadFrame = async (index: number) => {
    setBusy(`frame-${index}`); setError(null)
    try { await performDownload(index) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to download this frame.') }
    finally { setBusy(null) }
  }
  const downloadAll = async () => {
    setBusy('all'); setError(null)
    try { for (const frame of completedFrames) await performDownload(frame.index) }
  ...
  return <PageShell><header><p className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--brand-primary)]">Room summary</p><h1 className="mt-2 text-3xl font-black">{roomName}</h1><p className="mt-2 text-stone-500">{completedFrameIds.length} of {templates.length} frames completed</p><div aria-label={`Room completion: ${completedFrameIds.length} of ${templates.length}`} aria-valuemax={templates.length} aria-valuemin={0} aria-valuenow={completedFrameIds.length} className="mt-4 h-2 overflow-hidden rounded-full bg-stone-200" role="progressbar"><div className="h-full bg-[var(--brand-primary)] transition-[width]" style={{ width: `${templates.length ? completedFrameIds.length / templates.length * 100 : 0}%` }} /></div></header>
