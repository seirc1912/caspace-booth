...
export function PhotoLibraryDock({ photos, usage, activePhotoId, onSelect, onDelete, onImageError }: PhotoLibraryDockProps) {
  ...
  return <section aria-label="Photo library" className="fixed inset-x-0 bottom-[calc(4.75rem+env(safe-area-inset-bottom))] z-20 border-t border-stone-200 bg-white/95 px-3 py-2 shadow-[0_-12px_30px_rgba(28,25,23,0.08)] backdrop-blur-xl md:static md:rounded-[1.5rem] md:border md:p-4 md:shadow-sm">
    <div className="mx-auto max-w-4xl">
      <div className="mb-2"><h2 className="text-sm font-bold">Photo Library</h2><p className="text-[11px] text-stone-500">Photos from Capture One appear automatically</p></div>
      ...
      })}</div> : <div className="grid min-h-20 w-full place-items-center rounded-2xl border-2 border-dashed border-sky-200 bg-sky-50/60 px-4 text-center text-sm font-bold text-sky-700">Waiting for photos from Capture One…</div>}
    </div>
    {selectedPhoto ? <div className="fixed inset-0 z-50 flex items-end bg-stone-950/45 p-0 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-6" role="dialog" aria-modal="true" aria-label="Photo options"><button aria-label="Close photo options" className="absolute inset-0" onClick={() => setSelected(null)} type="button" /><div className="relative w-full rounded-t-[2rem] bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl sm:max-w-md sm:rounded-[2rem]"><div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-stone-200" /><img alt={selectedPhoto.alt} className="max-h-[55dvh] w-full rounded-2xl bg-[linear-gradient(45deg,#eee_25%,transparent_25%),linear-gradient(-45deg,#eee_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#eee_75%),linear-gradient(-45deg,transparent_75%,#eee_75%)] bg-[length:20px_20px] object-contain" onError={() => onImageError(`${selectedPhoto.alt || 'This image'} could not be displayed.`)} src={selectedPhoto.previewSrc ?? selectedPhoto.src} /><div className="mt-3 grid gap-2"><button className="min-h-12 rounded-xl bg-rose-50 font-bold text-rose-600" onClick={() => { onDelete(selectedPhoto.id); setSelected(null) }} type="button">Delete</button><button className="col-span-2 min-h-12 rounded-xl font-bold text-stone-500" onClick={() => setSelected(null)} type="button">Done</button></div></div></div> : null}
  </section>
}
