...
export function CropToolbar({ fit, filter, onCancel, onDone, onFilterChange, onFitChange, onReset, positionX, positionY, zoom, onPositionXChange, onPositionYChange, onZoomChange }: CropToolbarProps) {
  ...
  return <div className="bg-white px-3 pt-1 md:fixed md:inset-x-0 md:bottom-5 md:z-30 md:bg-transparent md:px-3 md:pt-0"><div className="mx-auto grid max-w-xl gap-1"><div className="grid gap-0.5 rounded-2xl bg-white p-1 shadow-xl md:bg-white/95 md:backdrop-blur-xl"><SliderRow id="crop-zoom" label="Zoom" max={maximumUserPhotoZoom} min={minimumUserPhotoZoom} onChange={onZoomChange} value={zoom} valueLabel={`${Math.round(zoom * 100)}%`} /><SliderRow id="crop-position-x" label="Left ↔ Right" max={1} min={-1} onChange={onPositionXChange} value={positionX} valueLabel={positionLabel(positionX)} /><SliderRow id="crop-position-y" label="Up ↕ Down" max={1} min={-1} onChange={onPositionYChange} value={positionY} valueLabel={positionLabel(positionY)} /></div><div className="flex flex-wrap items-center justify-center gap-1.5"><div className="flex items-center gap-0.5 rounded-[28px] bg-stone-950 p-1 shadow-xl md:bg-stone-950/88 md:backdrop-blur-xl" role="group" aria-label="Photo filter"><button aria-pressed={filter === 'none'} className={chip(filter === 'none')} onClick={() => onFilterChange('none')} type="button">Original</button><button aria-pressed={filter === 'grayscale'} className={chip(filter === 'grayscale')} onClick={() => onFilterChange('grayscale')} type="button">B&amp;W</button></div><div className="flex items-center gap-0.5 rounded-[28px] bg-stone-950 p-1 shadow-xl md:bg-stone-950/88 md:backdrop-blur-xl" role="group" aria-label="Crop fit"><button aria-pressed={fit === 'contain'} className={chip(fit === 'contain')} onClick={() => onFitChange('contain')} type="button">Contain</button><button aria-pressed={fit === 'cover'} className={chip(fit === 'cover')} onClick={() => onFitChange('cover')} type="button">Cover</button><button className={chip(false)} onClick={onReset} type="button">Reset</button></div><div className="flex items-center gap-0.5 rounded-[28px] bg-white p-1 shadow-xl md:bg-white/95 md:backdrop-blur-xl"><button className="min-h-11 rounded-full px-3 text-xs font-bold text-stone-500" onClick={onCancel} type="button">Cancel</button><button className="min-h-11 rounded-full bg-[var(--brand-secondary)] px-4 text-xs font-black text-white shadow-sm" onClick={onDone} type="button">Done</button></div></div></div></div>
}
