...
export function EditorToolbar({ canContinue, filter, filterDisabled = false, allBwEnabled = false, onDownload, onFilterChange, onToggleAllBw, onSkip, downloading = false, onPrevious, previousDisabled = false, skipDisabled = false }: EditorToolbarProps) {
  return <nav aria-label="Editor actions" className="border-t border-stone-200 bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-xl md:fixed md:inset-x-0 md:bottom-0 md:z-20 md:px-3 md:pb-[max(0.75rem,env(safe-area-inset-bottom))] md:pt-2">
    <div className="mx-auto max-w-3xl md:flex md:items-stretch md:gap-2">
      ...
      <div className="mt-0.5 md:mt-0 md:w-44">
        {onDownload ? <button aria-busy={downloading} aria-label="Save Photo" className={`${primaryActionClass} w-full bg-stone-950`} disabled={!canContinue || downloading} onClick={onDownload} type="button">{downloading ? 'Preparing' : 'Save Photo'}</button> : null}
      </div>
    </div>
  </nav>
}
