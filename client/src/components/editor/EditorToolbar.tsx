...
const primaryActionClass = 'grid min-h-12 min-w-0 place-items-center rounded-xl px-1.5 text-[11px] font-bold text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40 md:min-h-14 md:px-2 md:text-sm md:shadow-none'
...
export function EditorToolbar({ canContinue, canOrder, filter, filterDisabled = false, onDownload, onFilterChange, onNext, onSkip, onSave, nextLabel = 'Order', downloading = false, onPrevious, previousDisabled = false, skipDisabled = false, saved = false, progressLabel = null }: EditorToolbarProps) {
  return <nav aria-label="Editor actions" className="border-t border-stone-200 bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur-xl md:fixed md:inset-x-0 md:bottom-0 md:z-20 md:px-3 md:pb-[max(0.75rem,env(safe-area-inset-bottom))] md:pt-2">
    <div className="mx-auto max-w-3xl md:flex md:items-stretch md:gap-2">
      <div className="grid grid-cols-[minmax(0,0.8fr)_minmax(0,1.6fr)_minmax(0,0.8fr)] gap-1 md:flex-1">
        ...
        <div className="grid grid-cols-2 rounded-xl bg-stone-100 p-0.5"><button aria-label="Original photo" aria-pressed={filter === 'none'} className={filterActionClass(filter === 'none')} disabled={filterDisabled || downloading} onClick={() => onFilterChange('none')} type="button">Original</button><button aria-label="Black and white photo" aria-pressed={filter === 'grayscale'} className={filterActionClass(filter === 'grayscale')} disabled={filterDisabled || downloading} onClick={() => onFilterChange('grayscale')} type="button">B&amp;W</button></div>
