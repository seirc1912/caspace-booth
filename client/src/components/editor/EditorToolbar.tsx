...
export function EditorToolbar({ canContinue, onAutoFill, onClear, onNext, onShuffle, onDownload, nextLabel = 'Preview', downloading = false, onPrevious, previousDisabled = false }: EditorToolbarProps) {
  return (
    <nav aria-label="Editor actions" className="fixed inset-x-0 bottom-0 z-20 border-t border-stone-200 bg-white/95 px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl">
      <div className={`mx-auto grid max-w-3xl gap-1 ${onPrevious ? 'grid-cols-6' : onDownload ? 'grid-cols-5' : 'grid-cols-4'}`}>
        {onPrevious ? <button className="grid min-h-14 place-items-center rounded-xl px-1 text-xs font-semibold text-stone-600 hover:bg-stone-100 disabled:cursor-not-allowed disabled:opacity-35" disabled={previousDisabled || downloading} onClick={onPrevious} type="button"><Icon name="back" /><span>Previous</span></button> : null}
