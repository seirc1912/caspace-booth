...
export function TemplateElementLayer({ template }: TemplateElementLayerProps) {
  return template.elements.filter((element) => element.visible).map((element) => {
    ...
    if ((element.type === 'logo' || element.type === 'sticker') && element.assetUrl) return <img alt="" className="pointer-events-none absolute object-contain" key={element.id} src={element.assetUrl} style={style} />
